...
function Dashboard() {
    const { notes, searchInput, pagination, dialogState, handleAddNotes, handleUpdateNote, handleDeleteNote, handleChangePage, handleChangeRowsPerPage, handleStatus, handleCreateDialog, handleDeleteDialog } = useContext(NotesContext);
    const { createNotesDialog, deleteNotesDialog } = dialogState;
    ...
    const handleSubmit = async (values, { resetForm }) => {
        if (editData) {
            handleUpdateNote({ ...editData, ...values });
        } else {
            const timestamp = new Date().getTime();
            const uniqueId = notes.length + 1 + "_" + timestamp;
            handleAddNotes({ id: uniqueId, ...values });
        }
        handleDialogClose();
        resetForm();
    };
            ...
            <div>
                <Grid container width='90%' marginInline='auto'>
                    ...
                    >
                        ...
                        <DialogActions>
                            <Button onClick={() => handleDeleteDialog(false)} sx={{
                                backgroundColor: "lightgrey", borderRadius: 1, color: "black", textTransform: 'capitalize', "&:hover": {
                                    backgroundColor: "gray", color: '#fff'
                                }
                            }}>Cancel</Button>
