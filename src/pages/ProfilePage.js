...
const ProfilePage = () => {
    const { updateUserDialog, setUpdateUserDialog, currentUser, handleUpdateUser } = useContext(UserContext);
    const [editData, setEditData] = useState(null);

    const handleDialogOpen = (data = null) => {
        setEditData(data || currentUser);
    ...
    const handleSubmit = (values, { resetForm }) => {
        handleUpdateUser({ ...editData, ...values });
        handleDialogClose();
        resetForm();
    };
                    ...
                    <CardContent>
                        <Grid container spacing={2} alignItems="center" direction='column'>
                            <Button
                                ...
                                onClick={() => handleDialogOpen(currentUser)}
                            >
                                Update Profile
                            </Button>
                        </Grid>
